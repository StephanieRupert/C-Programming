...
saveadv()
{
	...
	getsCheck(fgets(username,sizeof(username),stdin));
...
restore()
{
	...
	readCheck(fread( &turns, sizeof(int), 1, restfd));
	readCheck(fread( &loc, sizeof(int), 1, restfd));
	readCheck(fread( &oldloc, sizeof(int), 1, restfd));
	readCheck(fread( &oldloc2, sizeof(int), 1, restfd));
	readCheck(fread( &newloc, sizeof(int), 1, restfd));	/* location variables */
	readCheck(fread( &cond[0], sizeof(int), MAXLOC, restfd));			/* location status	*/
	readCheck(fread( &place[0], sizeof(int), MAXOBJ, restfd));		/* object location	*/
	readCheck(fread( &fixed[0], sizeof(int), MAXOBJ, restfd));		/* second object loc	*/
	readCheck(fread( &visited[0], sizeof(int), MAXLOC, restfd));		/* >0 if has been here	*/
	readCheck(fread( &prop[0], sizeof(int), MAXOBJ, restfd));			/* status of object	*/
	readCheck(fread( &tally, sizeof(int), 1, restfd));
	readCheck(fread( &tally2, sizeof(int), 1, restfd));		/* item counts		*/
	readCheck(fread( &limit, sizeof(int), 1, restfd));			/* time limit		*/
	readCheck(fread( &lmwarn, sizeof(int), 1, restfd));			/* lamp warning flag	*/
	readCheck(fread( &wzdark, sizeof(int), 1, restfd));
	readCheck(fread( &closing, sizeof(int), 1, restfd));
	readCheck(fread( &closed, sizeof(int), 1, restfd));	/* game state flags	*/
	readCheck(fread( &holding, sizeof(int), 1, restfd));		/* count of held items	*/
	readCheck(fread( &detail, sizeof(int), 1, restfd));			/* LOOK count		*/
	readCheck(fread( &knfloc, sizeof(int), 1, restfd));			/* knife location	*/
	readCheck(fread( &clock, sizeof(int), 1, restfd));
	readCheck(fread( &clock2, sizeof(int), 1, restfd));
	readCheck(fread( &panic, sizeof(int), 1, restfd));	/* timing variables	*/
	readCheck(fread( &dloc[0], sizeof(int), DWARFMAX, restfd));			/* dwarf locations	*/
	readCheck(fread( &dflag, sizeof(int), 1, restfd));			/* dwarf flag		*/
	readCheck(fread( &dseen[0], sizeof(int), DWARFMAX, restfd));		/* dwarf seen flag	*/
	readCheck(fread( &odloc[0], sizeof(int), DWARFMAX, restfd));		/* dwarf old locations	*/
	readCheck(fread( &daltloc, sizeof(int), 1, restfd));		/* alternate appearance	*/
	readCheck(fread( &dkill, sizeof(int), 1, restfd));			/* dwarves killed	*/
	readCheck(fread( &chloc, sizeof(int), 1, restfd));
	readCheck(fread( &chloc2, sizeof(int), 1, restfd));		/* chest locations	*/
	readCheck(fread( &bonus, sizeof(int), 1, restfd));			/* to pass to end	*/
	readCheck(fread( &numdie, sizeof(int), 1, restfd));			/* number of deaths	*/
	readCheck(fread( &object1, sizeof(int), 1, restfd));		/* to help intrans.	*/
	readCheck(fread( &gaveup, sizeof(int), 1, restfd));			/* 1 if he quit early	*/
	readCheck(fread( &foobar, sizeof(int), 1, restfd));			/* fie fie foe foo...	*/
	if (fclose(restfd)  ==  -1) {
